...
const ODP=(t,p,o)=>{try{Object.defineProperty(t,p,o)}catch(e){console.trace("[Web Bypass] Couldn't define",p)}},
...
domainBypass=(domain,f)=>{
	...
	if(typeof f!="function")
	{
		alert("Web Bypass: Bypass for "+domain+" is not a function")
	}
	...
	{
		console.error("[Web Bypass] Invalid domain:",domain)
	}
},
hrefBypass=(regex,f)=>{
	if(bypassed)
...
ensureDomLoaded(()=>{
	...
	domainBypass(/linkvertise\.(com|net)|link-to\.net/,()=>insertInfoBox(WEB_BYPASS_INTERNAL_VERSION>=9?"{{msg.infoLinkvertise}}":"We're not allowed to bypass this website but we have negotiated the removal of their most annoying steps."))
		...
		if(typeof app_vars=="object")
		{
			...
			let iT=setInterval(()=>{
				if(document.documentElement.hasAttribute("{{channel.adlinkfly_target}}"))
				{
					...
					if(t=="")
					{
						if(WEB_BYPASS_INTERNAL_VERSION>=10&&bypassClipboard.substr(0,8)=="psarips:")
						{
